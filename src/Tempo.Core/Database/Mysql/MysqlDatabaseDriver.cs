namespace Tempo.Core.Database.Mysql
{
    ...
    /// <summary>MySQL implementation of <see cref="DatabaseDriverBase"/>.</summary>
    public class MysqlDatabaseDriver : DatabaseDriverBase
    {
        ...
        public MysqlDatabaseDriver(DatabaseSettings settings)
        {
            ...
            _Settings = settings;
            int port = settings.Port == 0 ? 3306 : settings.Port;
            ConnectionString =
                "Server=" + (settings.Server ?? "localhost") + ";" +
                "Port=" + port + ";" +
                "Database=" + (settings.DatabaseName ?? "tempo") + ";" +
                "Uid=" + (settings.Username ?? "root") + ";" +
                "Pwd=" + (settings.Password ?? string.Empty) + ";" +
                "DefaultCommandTimeout=" + settings.CommandTimeoutSeconds + ";";
        ...
        {
            IReadOnlyList<SchemaMigration> migrations = MysqlSchema.All();
        ...
        private async Task<DataTable> ExecuteInternalAsync(IEnumerable<string> queries, bool isTransaction, CancellationToken token)
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(ConnectionString))
            {
                ...
                MySqlTransaction? tx = isTransaction ? (MySqlTransaction)await connection.BeginTransactionAsync(token).ConfigureAwait(false) : null;
                try
                {
